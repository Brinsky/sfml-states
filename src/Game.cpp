...
void Game::changeState(std::unique_ptr<GameState> a_state)
{
	if(!states.empty())
	{
...
void Game::pushState(std::unique_ptr<GameState> a_state)
{
	if(!states.empty())
	{
		states.top()->pause();
